...
export function MorningBriefingCard({ workspaceId, className }: { workspaceId: string; className?: string }) {
  ...
  return (
    <section
      className={cn("bg-card border border-border relative overflow-hidden rounded-2xl border border-foreground/10 p-5 shadow-lift sm:p-6", className)}
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold text-muted-foreground">إحاطة {eva?.name ?? "أمَل"} اليومية</p>
        ...
        <>
          <div className="mt-5 grid grid-cols-3 gap-2 [&>*]:min-w-0">
            {[
              ...
              <div key={s.k} className="rounded-xl border border-border/50 bg-secondary/40 px-3 py-3 sm:px-4">
                <p className="truncate text-[10px] text-foreground/60 sm:text-[11px]">{s.k}</p>
                <p className="font-display text-xl font-black text-foreground">{s.v.toLocaleString("ar-EG")}</p>
              </div>
            ))}
          </div>
          ...
          {data.attention.length ? (
            <ul className="mt-4 space-y-1.5">
              {data.attention.map((a) => (
                <li key={a} className="flex items-center gap-2 rounded-xl bg-amber/15 px-3 py-2 text-xs font-semibold text-foreground">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2 [&>*]:min-w-0">
              ...
              {data.todayPosts.length ? (
                <ul className="mt-2 space-y-1.5">
                  {data.todayPosts.map((p) => (
                    <li key={p.id}>
                      <Link to="/app/calendar" className="flex items-center gap-2 rounded-xl border border-border/40 bg-secondary/30 p-1.5 pe-3 text-xs text-foreground transition-colors hover:bg-background/12">
                        {p.image ? (
            ...
            <div>
              ...
              {data.ideas.length ? (
                ...
                <p className="mt-2 text-xs text-foreground/60">أفكار اليوم ستظهر بعد تحليل علامتك.</p>
              )}
            </div>
          </div>
        </>
